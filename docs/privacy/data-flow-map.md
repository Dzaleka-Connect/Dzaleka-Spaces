...
| Enquiry contact + messages + attachments | listing enquiry, threads         | `enquiries`, `enquiry_messages`, `message-private` bucket | thread participants + staff                  |
...
| DzalekaPay reconciliation                | payment UUID, provider API/event | `dzalekapay_reconciliations`, `dzalekapay_webhook_events` | payment parties; service route; MFA staff    |
...
- **DzalekaPay** — server-side transaction status/amount read and signed
  transaction events. Dzaleka Spaces sends only the transaction UUID and does
  not persist the payer phone or raw provider payload.
...
- DzalekaPay API keys, webhook secrets, full payer phone numbers or upstream
  provider payloads in application data.
