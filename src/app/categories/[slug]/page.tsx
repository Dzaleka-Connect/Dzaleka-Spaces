...
const CATEGORY_INTROS: Record<string, string> = {
  community_venue: "Halls and community venues for meetings, ceremonies, trainings and events.",
  training_space: "Rooms equipped for courses and workshops — many with electricity and internet.",
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8">
      <div>
        <h1 className="text-3xl font-bold">{category.label}s</h1>
        <p className="mt-1 max-w-2xl text-muted-foreground">{CATEGORY_INTROS[category.value]}</p>
      </div>
      ...
      {listings.length === 0 ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              New {category.label.toLowerCase()}s appear here as soon as they pass field
              verification.
            </EmptyDescription>
          </EmptyHeader>
