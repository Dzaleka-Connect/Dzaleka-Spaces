...
import { OccupantConfirmPaymentButton } from "./confirm-button";
              ...
              {payments.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Use the panel on the right to report one.
                  </p>
                </div>
              ) : (
                <Table>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button
                              variant="ghost"
                              size="icon"
                              render={<Link href={`/account/payments/${p.id}`} />}
                              nativeButton={false}
                            >
                              <Eye className="size-4" />
                            </Button>
                            {p.status === "pending_confirmation" && !p.payerConfirmedAt && (
                              <OccupantConfirmPaymentButton paymentId={p.id} />
                            )}
                          </div>
                        </TableCell>
                      </TableRow>
        ...
        <div>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                If you made a cash payment or mobile money transfer, log it here so your provider
                can confirm it.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {activeOccupancies.length === 0 ? (
                <div className="text-sm text-muted-foreground text-center py-6">
                  No active occupancy records. You must be added to an occupancy by a provider
                  before reporting payments.
                </div>
              ) : (
                <ReportPaymentForm
